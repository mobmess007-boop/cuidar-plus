...
const AdicionarPressao = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        pulse: '',
        notes: '',
        measured_at: new Date().toISOString().slice(0, 16) // Format for datetime-local
    });
        ...
        <Layout title="Nova Medição" showBack>
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    <div>
                        <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Sistólica</label>
                        <input
                            type="number"
                            name="systolic"
                            required
                    ...
                    <div>
                        <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Diastólica</label>
                        <input
                            type="number"
                            name="diastolic"
                            required
